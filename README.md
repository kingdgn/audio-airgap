...
接收端现在加入了软判决 + CRC32 引导纠错。普通硬判决失败后，解码器会保留每个 4-FSK 符号的最强、次强和第三强候选，只对最像帧头的前 6 个对齐偏移做受限搜索；每个偏移最多尝试 80000 组组合，最多修正 5 个符号。只有修正后的整帧 CRC32 完全匹配，才会计为有效帧。

手机网页会显示“纠错恢复”数量；如果这个数持续增加，说明环境有少量误码但仍可救回。如果仍然只有“错误片段”，优先改用更稳的参数，例如 `--channels 8 --symbol-ms 20 --chunk-size 256`，或只补播缺失 index。
...
- 抗噪优化：接收端优先检测 880 Hz pilot 起点，并用目标 4-FSK 频点能量占比过滤普通杂音。
...
| `--symbol-ms` | 每个符号持续时间 | 10ms 更快，20ms 更稳但更慢；有明显杂音时优先改成 20ms |
| `--repeat-data` | 全量数据重复播放次数 | 默认 1；不要轻易加倍，建议先传一遍再按缺片补播 |

推荐先用 `--chunk-size 512 --channels 12 --symbol-ms 10` 跑通；若缺片很少，再尝试 `--chunk-size 1024 --channels 16 --symbol-ms 10` 来缩短音频时间。若现场有持续杂音，优先用 `--channels 8 --symbol-ms 20 --chunk-size 256` 或 `--channels 12 --symbol-ms 20 --chunk-size 512`。
## 快速开始
...
如果页面持续显示错误片段，先不要反复猜参数。手机端停止录制后点击 **下载录音 WAV**，把手机实际听到的音频保存下来；同时可以点击 **下载诊断 JSON** 保存当前参数、错误片段数和日志。用这个 WAV 可以在电脑端离线分析频谱、音量削波、pilot 是否存在，以及到底是分段失败还是 4-FSK 判决失败。
...
- 优先把 `--symbol-ms` 从 10 改成 20，牺牲速度换稳定性。
- 降低 `--channels`，例如从 16 降到 12；很吵时用 8。
- 把手机接收页的 `调制音阈值` 作为触发阈值：杂音误触发就调高，例如 0.012 -> 0.018；听得到播放但不触发就调低，例如 0.012 -> 0.008。
- 增大发送端音量，但不要让扬声器破音；手机距离扬声器 10-30 cm 通常更稳。
- 使用缺片补播，不要整段重传。
